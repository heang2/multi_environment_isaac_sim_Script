...
3) The scene will be generated under DualArmSortingCellComplex.
"""
...
def build_scene_2(parent: str):
    """Dual-arm sorting cell with shuttle robot and conveyor."""
    _add_grid_env(parent)
    _add_light(parent + "/SunLight", intensity=3500, translate=(0, 0, 14), rotate_xyz=(10, -55, 0))
    _add_dome_light(parent + "/DomeLight", intensity=650)

    _add_robot("franka", parent + "/FrankaLeft", pos=(-2.8, 1.8, 0.0), yaw_deg=0)
    _add_robot("franka", parent + "/FrankaRight", pos=(2.8, 1.8, 0.0), yaw_deg=180)
    _add_robot("jetbot", parent + "/JetbotShuttle", pos=(0.0, -3.8, 0.0), yaw_deg=0)

    _add_conveyor(parent + "/Conveyor", center=(0.0, 0.0, 0.55), length=7.2, width=1.0)
    _add_worktable(parent + "/TableLeft", center=(-3.0, 3.2, 0.9))
    _add_worktable(parent + "/TableRight", center=(3.0, 3.2, 0.9))

    for i in range(8):
        _add_dynamic_box(
            parent + f"/Parts/part_{i}",
            pos=(-2.6 + i * 0.75, 0.0, 0.82 + 0.04 * (i % 2)),
            scale=(0.22, 0.22, 0.18),
            color=(0.2 + 0.08 * i, 0.6, 0.9 - 0.07 * i),
        )

    for i, xy in enumerate([(-5.0, -1.0), (-5.0, 1.0), (5.0, -1.0), (5.0, 1.0)]):
        _add_visual_box(parent + f"/Bins/bin_{i}", pos=(xy[0], xy[1], 0.35), scale=(0.8, 0.8, 0.7), color=(0.15, 0.35, 0.8))

# =========================
# top-level build
# =========================
CLEAR_OLD_GENERATED = True
ROOT_PATH = "/World/DualArmSortingCellComplex"
SCENE_DISPLAY_NAME = "DualArmSortingCellComplex"
