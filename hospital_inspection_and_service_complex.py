"""
HospitalInspectionAndServiceComplex
...
3) The scene will be generated under HospitalInspectionAndServiceComplex.
"""
...
def build_scene_3(parent: str):
    """Hospital or room inspection: mobile base + quadruped + narrow passage obstacles."""
    _add_env(parent, "hospital")
    _add_light(parent + "/SunLight", intensity=3200, translate=(0, 0, 12), rotate_xyz=(15, -40, 0))
    _add_dome_light(parent + "/DomeLight", intensity=700)

    _add_robot("nova_carter", parent + "/InspectorBase", pos=(-3.5, -2.0, 0.0), yaw_deg=30)
    _add_robot("spot", parent + "/SpotScout", pos=(2.5, -1.0, 0.0), yaw_deg=-20)
    _add_robot("franka", parent + "/BedsideArm", pos=(4.5, 2.2, 0.0), yaw_deg=180)

    for i, center in enumerate([(-1.0, 2.0, 0.45), (2.0, 2.0, 0.45), (-1.0, -3.0, 0.45)]):
        _add_worktable(parent + f"/Beds/bed_{i}", center=center, table_scale=(2.2, 0.95, 0.18), leg_h=0.6)

    _add_barrier_line(parent + "/CorridorA", start_xy=(-4.0, 4.5), count=8, spacing=1.0, along="x")
    _add_barrier_line(parent + "/CorridorB", start_xy=(-4.0, -4.5), count=8, spacing=1.0, along="x")

    for i, pos in enumerate([(0.0, -1.5, 0.35), (0.8, -1.2, 0.7), (1.6, -1.6, 0.35)]):
        _add_dynamic_box(parent + f"/MedicalSupplies/supply_{i}", pos=pos, scale=(0.4, 0.4, 0.4), color=(0.95, 0.95 - i * 0.2, 0.3))

# =========================
# top-level build
# =========================
CLEAR_OLD_GENERATED = True
ROOT_PATH = "/World/HospitalInspectionAndServiceComplex"
SCENE_DISPLAY_NAME = "HospitalInspectionAndServiceComplex"
