"""
OfficeDeliveryMultiRobotComplex
...
3) The scene will be generated under OfficeDeliveryMultiRobotComplex.
"""
...
def build_scene_4(parent: str):
    """Office delivery fleet: holonomic + differential + package clutter."""
    _add_env(parent, "office")
    _add_light(parent + "/SunLight", intensity=2800, translate=(0, 0, 14), rotate_xyz=(5, -50, 0))
    _add_dome_light(parent + "/DomeLight", intensity=750)

    _add_robot("jetbot", parent + "/JetbotCourier", pos=(-4.0, -3.0, 0.0), yaw_deg=45)
    _add_robot("create3", parent + "/Create3Courier", pos=(0.0, -2.5, 0.0), yaw_deg=0)
    _add_robot("kaya", parent + "/KayaCarrier", pos=(4.0, -3.0, 0.0), yaw_deg=-45)
    _add_robot("dingo", parent + "/DingoPatrol", pos=(0.0, 3.0, 0.0), yaw_deg=180)

    for i, center in enumerate([(-5.5, 0.0, 0.78), (-1.8, 0.2, 0.78), (1.8, -0.2, 0.78), (5.5, 0.0, 0.78)]):
        _add_worktable(parent + f"/Desks/desk_{i}", center=center, table_scale=(1.8, 1.0, 0.12), leg_h=0.7)

    for i in range(10):
        x = -4.5 + (i % 5) * 2.2
        y = 1.8 if i < 5 else -0.8
        z = 0.25 if i % 2 == 0 else 0.5
        _add_dynamic_box(parent + f"/Packages/pkg_{i}", pos=(x, y, z), scale=(0.35, 0.35, 0.35), color=(0.7, 0.45 + 0.03 * i, 0.2))

    _add_barrier_line(parent + "/OfficeDivider", start_xy=(-6.0, -5.0), count=12, spacing=1.0, along="x")

# =========================
# top-level build
# =========================
CLEAR_OLD_GENERATED = True
ROOT_PATH = "/World/OfficeDeliveryMultiRobotComplex"
SCENE_DISPLAY_NAME = "OfficeDeliveryMultiRobot"
