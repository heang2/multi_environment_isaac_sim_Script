...
3) The scene will be generated under WarehouseMixedFleetComplex.
"""
...
def build_scene_1(parent: str):
    """Full warehouse: AMR + forklift + manipulator + pallet congestion."""
    _add_env(parent, "full_warehouse")
    _add_light(parent + "/SunLight", intensity=4000, translate=(0, 0, 18), rotate_xyz=(20, -45, 0))
    _add_dome_light(parent + "/DomeLight", intensity=500)

    _add_robot("nova_carter", parent + "/NovaCarter", pos=(2.0, -4.5, 0.0), yaw_deg=90)
    _add_robot("forklift_c", parent + "/Forklift", pos=(-5.5, -2.0, 0.0), yaw_deg=0)
    _add_robot("franka", parent + "/FrankaStation", pos=(6.5, 3.5, 0.0), yaw_deg=-90)
    _add_robot("iw_hub_sensors", parent + "/IWHub", pos=(0.5, 6.0, 0.0), yaw_deg=180)

    for i, xy in enumerate([(-2, 1), (-2, 2.6), (-2, 4.2), (3.6, -1.0), (4.8, -1.0)]):
        _add_pallet_stack(parent + f"/Stacks/stack_{i}", xy, layers=3 + (i % 2))

    _add_barrier_line(parent + "/BarriersA", start_xy=(-6.5, 5.2), count=7, spacing=1.2, along="x")
    _add_barrier_line(parent + "/BarriersB", start_xy=(7.0, -4.5), count=6, spacing=1.1, along="y")

    for i, pos in enumerate([(1.2, 1.1, 0.25), (1.8, 1.4, 0.55), (2.3, 1.2, 0.85)]):
        _add_dynamic_box(parent + f"/FallenBoxes/box_{i}", pos=pos, scale=(0.45, 0.45, 0.45), color=(0.9, 0.2 + 0.2 * i, 0.2))

# =========================
# top-level build
# =========================
CLEAR_OLD_GENERATED = True
ROOT_PATH = "/World/WarehouseMixedFleetComplex"
SCENE_DISPLAY_NAME = "WarehouseMixedFleetComplex"
