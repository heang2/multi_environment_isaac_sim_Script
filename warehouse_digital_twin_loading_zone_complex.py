"""
WarehouseDigitalTwinLoadingZoneComplex
...
3) The scene will be generated under WarehouseDigitalTwinLoadingZoneComplex.
"""
...
def build_scene_6(parent: str):
    """Small warehouse digital twin + mixed fleet + loading zone."""
    _add_env(parent, "small_warehouse_digital_twin")
    _add_light(parent + "/SunLight", intensity=3600, translate=(0, 0, 15), rotate_xyz=(12, -50, 0))
    _add_dome_light(parent + "/DomeLight", intensity=600)

    _add_robot("iw_hub_sensors", parent + "/HubSensors", pos=(-3.5, -4.0, 0.0), yaw_deg=45)
    _add_robot("leatherback", parent + "/Leatherback", pos=(2.0, -3.5, 0.0), yaw_deg=0)
    _add_robot("forklift_b", parent + "/ForkliftB", pos=(6.5, 0.0, 0.0), yaw_deg=90)
    _add_robot("jackal", parent + "/JackalScout", pos=(-6.0, 2.5, 0.0), yaw_deg=-90)
    _add_robot("franka", parent + "/PackingArm", pos=(4.5, 4.2, 0.0), yaw_deg=-135)

    _add_worktable(parent + "/PackingStation", center=(3.8, 5.8, 0.9), table_scale=(2.0, 1.0, 0.15), leg_h=0.75)
    _add_conveyor(parent + "/LoadingConveyor", center=(0.5, 5.0, 0.55), length=5.8, width=1.1)

    for i, xy in enumerate([(-1.5, 1.8), (-0.2, 1.8), (1.1, 1.8), (2.4, 1.8)]):
        _add_pallet_stack(parent + f"/Inbound/stack_{i}", xy, layers=2 + (i % 3))

    for i in range(6):
        _add_dynamic_box(
            parent + f"/Outbound/box_{i}",
            pos=(-2.2 + i * 0.7, 5.0, 0.82),
            scale=(0.28, 0.28, 0.24),
            color=(0.2, 0.75 - 0.07 * i, 0.95),
        )
...
ROOT_PATH = "/World/WarehouseDigitalTwinLoadingZoneComplex"
SCENE_DISPLAY_NAME = "WarehouseDigitalTwinLoadingZone"
